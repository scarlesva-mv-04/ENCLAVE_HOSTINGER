...
$(document).ready(function () {
    function updateHeader() {
        ...
        const userLink = isLoggedIn ? "/user_options" : "/login";
        const propertiesLinkDesktop = isLoggedIn ? "<li><a href='/properties'>Propiedades</a></li>" : "";
        const propertiesLinkMobile = isLoggedIn ? "<a href='/properties' class='resaltar encabezado'>INICIO</a>" : "<a href='/' class='resaltar encabezado'>INICIO</a>";
            ...
            $("header.default").html(`
                    <nav>
                        <ul>
                            ...
                            <li><a href="/our-history">Nuestra historia</a></li>
                            ${propertiesLinkDesktop}
                            <li class="user">
                                <a href="${userLink}"><img src="../images/icons/ri_user-5-line.svg" alt="User"></a>
                            </li>
                        </ul>
                    </nav>
            `);
    ...
    <div id="contenedor-menu" class="menu-lateral">
        ...
        <div class="menu">
            <nav>
                ${propertiesLinkMobile}
                <a href="/about_us" class="resaltar encabezado">SOBRE NOSOSTROS</a>
                <a href="/testimonials" class="resaltar encabezado">TESTIMONIALS</a>
                <a href="/cita" class="resaltar encabezado">AGENDA UNA CITA</a>
            </nav>
        </div>
    </div>
            `);
...
$(document).ready(function () {
    ...
    $("#loginForm").submit(function (e) {
        ...
        if (isValid) {
            // Si es válido, enviamos el formulario manualmente
            this.submit(); // Usamos el método nativo del DOM, no el de jQuery
        }
    });
});
...
function toggleMenu() {
    const menu = $("div#contenedor-menu");
    const body = $("body");
...
$(document).ready(function () {
    // Restaurar estilos al escribir en campos individuales
    $("input[name='name'], input[name='surname'], input[name='dni'], input[name='email'], input[name='phone'], #datepicker").on("input", function () {
        $(this).removeClass("error").css("transition", "all 0.3s ease");
        let placeholderDefault = $(this).data("placeholder");
        if (placeholderDefault) {
            $(this).fadeOut(100, function () {
                $(this).attr("placeholder", placeholderDefault).fadeIn(300);
            });
        }
    });

    // Restaurar selección de hora
    $("#hours_avaliable").on("click", "div.hour", function () {
    ...
    $("form").submit(function (e) {
        ...
        // Validar DNI o NIE
        function validarDocumento(ident) {
            ident = ident.toUpperCase();
            const letras = "TRWAGMYFPDXBNJZSQVHLCKE";
            if (/^\d{8}[A-Z]$/.test(ident)) {
                return letras.charAt(parseInt(ident.substr(0, 8)) % 23) === ident[8];
            }
            if (/^[XYZ]\d{7}[A-Z]$/.test(ident)) {
                const mapa = { X: '0', Y: '1', Z: '2' };
                const nie = mapa[ident[0]] + ident.substr(1, 7);
                return letras.charAt(parseInt(nie) % 23) === ident[8];
            }
            return false;
        }
        ...
        if (nombre === "" || !regexNombre.test(nombre)) {
            $name.addClass("error").val("").attr("placeholder", "").fadeOut(100, function () {
                $(this).attr("placeholder", "Nombre inválido").fadeIn(300);
            });
            isValid = false;
        }
        ...
        const apellidos = $surname.val().trim();
        if (apellidos === "" || !regexNombre.test(apellidos)) {
            $surname.addClass("error").val("").attr("placeholder", "").fadeOut(100, function () {
                $(this).attr("placeholder", "Apellidos inválidos").fadeIn(300);
            });
            isValid = false;
        }
        ...
        if (!validarDocumento(dni)) {
            $dni.addClass("error").val("").attr("placeholder", "").fadeOut(100, function () {
                $(this).attr("placeholder", "DNI o NIE inválido").fadeIn(300);
            });
            isValid = false;
        }
        ...
        if (!regexEmail.test(email)) {
            $email.addClass("error").val("").attr("placeholder", "").fadeOut(100, function () {
                $(this).attr("placeholder", "Email inválido").fadeIn(300);
            });
            isValid = false;
        }
        ...
        if (!regexTelefono.test(phone)) {
            $phone.addClass("error").val("").attr("placeholder", "").fadeOut(100, function () {
                $(this).attr("placeholder", "Teléfono inválido").fadeIn(300);
            });
            isValid = false;
        }

        // Validar fecha
        const $fecha = $("#datepicker");
        const fecha = $fecha.val().trim();
        if (fecha === "") {
            $fecha.addClass("error").val("").attr("placeholder", "").fadeOut(100, function () {
                $(this).attr("placeholder", "Seleccione una fecha").fadeIn(300);
            });
            isValid = false;
        }

        // Validar hora
        const hora = $("#appointment_time").val().trim();
        if (hora === "") {
            $("#appointment_time").addClass("error");
            isValid = false;
        }

        if (isValid) this.submit(); // Envío nativo
    });

    // Restaurar placeholders al cargar
        ...
        const observer = new IntersectionObserver((entries, obs) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (bg) {
                        el.style.backgroundImage = `linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('${bg}')`;
